...
> 面向科研场景的智能文献管理、知识检索与研究分析助手。

AI Research Agent 是用于比赛展示与校招实践的科研知识智能体原型。它保留单篇深度分析能力，并增加论文库、多论文 RAG 知识问答、结构化研究报告、引用证据和可解释执行摘要。

本项目用于比赛展示，不是生产系统；不虚构用户数量、准确率、商业收入或企业落地案例。

## 比赛展示版本说明

AI Research Agent 不是简单的 PDF 总结工具，而是面向科研任务的知识智能体原型。用户以研究问题、论文资料和分析目标驱动 Agent：先理解问题，再规划检索策略，筛选论文证据，并生成可复核的研究结论与报告。

比赛展示版保留浏览器本地的角色化工作空间和基础 PWA 入口；同时新增 SQLite 论文库、RAG 问答历史和研究报告历史。论文原文与索引存放在本地 `work/` 目录，不会上传到前端。
